<?php
include_once(__DIR__ . '/../../config/db.php');
session_start();

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: ../../views/auth/login.php");
    exit();
}

$id_usuario  = $_SESSION['usuario_id'];
$tipo_user   = isset($_SESSION['id_tipo_usuario']) ? (int)$_SESSION['id_tipo_usuario'] : 3;
$redireccion = ($tipo_user == 2)
    ? '../../views/roles/asistente.php'
    : '../../views/roles/paciente.php';

// Recibir datos del slot seleccionado
$id_horario  = isset($_POST['id_horario'])  ? (int)$_POST['id_horario']  : 0;
$fecha_slot  = mysqli_real_escape_string($conexion, trim($_POST['fecha_slot']  ?? ''));
$hora_slot   = mysqli_real_escape_string($conexion, trim($_POST['hora_slot']   ?? ''));

// Validar que tengamos fecha y hora (siempre vienen del calendario dinámico)
if (empty($fecha_slot) || empty($hora_slot)) {
    echo "<script>alert('Error: Selecciona un horario válido.'); window.location.href='$redireccion';</script>";
    exit();
}

// Validar que la fecha no sea pasada (zona horaria México ya fijada en db.php)
if ($fecha_slot < date('Y-m-d')) {
    echo "<script>alert('Error: No puedes agendar en una fecha pasada.'); window.location.href='$redireccion';</script>";
    exit();
}

// Verificar si el paciente ya tiene cita activa futura
if ($tipo_user == 3) {
    $check = mysqli_query($conexion,
        "SELECT c.id_cita FROM cita c
         JOIN horario h ON c.id_horario = h.id_horario
         WHERE c.id_usuario = '$id_usuario'
           AND c.id_estado_cita IN (1, 4)
           AND h.fecha >= CURDATE()");
    if (mysqli_num_rows($check) > 0) {
        echo "<script>alert('Ya tienes una cita pendiente o confirmada. Cancélala primero.'); window.location.href='$redireccion';</script>";
        exit();
    }
}

// Buscar o crear el registro en la tabla horario
if ($id_horario > 0) {
    // El slot ya existe en BD — verificar que siga disponible
    $res_h = mysqli_query($conexion, "SELECT id_horario, estado, disponible FROM horario WHERE id_horario = '$id_horario'");
    $row_h = mysqli_fetch_assoc($res_h);
    if (!$row_h || $row_h['estado'] === 'ocupado' || (int)$row_h['disponible'] === 0) {
        echo "<script>alert('Este horario ya no está disponible. Elige otro.'); window.location.href='$redireccion';</script>";
        exit();
    }
} else {
    // Slot virtual — verificar que no exista ya ocupado o bloqueado para esa fecha/hora
    $hora_fin = date('H:i:s', strtotime($hora_slot) + 3600); // +1 hora
    $res_exist = mysqli_query($conexion,
        "SELECT id_horario, estado, disponible FROM horario
         WHERE fecha = '$fecha_slot' AND hora_inicio = '$hora_slot'");
    if ($row_exist = mysqli_fetch_assoc($res_exist)) {
        if ($row_exist['estado'] === 'ocupado' || (int)$row_exist['disponible'] === 0) {
            echo "<script>alert('Este horario ya fue reservado o está bloqueado. Elige otro.'); window.location.href='$redireccion';</script>";
            exit();
        }
        $id_horario = (int)$row_exist['id_horario'];
    } else {
        // Crear el horario en la BD
        mysqli_query($conexion,
            "INSERT INTO horario (fecha, hora_inicio, hora_fin, disponible, estado)
             VALUES ('$fecha_slot', '$hora_slot', '$hora_fin', 1, 'disponible')");
        $id_horario = (int)mysqli_insert_id($conexion);
    }
}

// Insertar la cita
$sql = "INSERT INTO cita (id_usuario, id_horario, id_estado_cita, fecha_registro)
        VALUES ('$id_usuario', '$id_horario', 1, NOW())";

if (mysqli_query($conexion, $sql)) {
    mysqli_query($conexion, "UPDATE horario SET estado = 'ocupado' WHERE id_horario = '$id_horario'");
    echo "<script>alert('¡Cita agendada con éxito!'); window.location.href='$redireccion';</script>";
} else {
    echo "<script>alert('Error al agendar: " . addslashes(mysqli_error($conexion)) . "'); window.location.href='$redireccion';</script>";
}
?>
